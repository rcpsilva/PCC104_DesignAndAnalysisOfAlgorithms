...
\begin{enumerate}
  ...
  \item Em quais linhas da sua implementação do método \texttt{partition} são feitas as comparações entre elementos do arranjo e o pivô? Qual o custo dessas comparações?

  \item Construa um \texttt{max-heap} para o array $A = [5,3,17,10,84,19,6,22,9,1]$. Mostre o heap em forma de árvore e em forma de array.

  \item Considere o algoritmo abaixo, que verifica se o arranjo $A[1\,..\,n]$ contém algum par de elementos iguais (isto é, algum elemento duplicado).
...
\end{enumerate}
  ...
  \item Considere o algoritmo recursivo abaixo, que retorna o maior elemento do arranjo $A[p\,..\,r]$ pelo método de divisão e conquista.

    \begin{center}
    \begin{minipage}{0.7\textwidth}
    \textbf{\textsc{Maximo}}$(A, p, r)$
    \begin{algorithmic}[1]
        \If{$p = r$}
            \State \Return $A[p]$
        \EndIf
        \State $q \gets \lfloor (p + r) / 2 \rfloor$
        \State $m_1 \gets \textsc{Maximo}(A, p, q)$
        \State $m_2 \gets \textsc{Maximo}(A, q + 1, r)$
        \If{$m_1 \geq m_2$}
            \State \Return $m_1$
        \Else
            \State \Return $m_2$
